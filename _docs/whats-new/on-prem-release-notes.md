...
| `appDiffView`       | _This Feature Flag is currently in Beta and only enabled for SaaS environments._ We will notify you when you can enable it for on-premises environments. <br>When enabled, and the application is out of sync, displays the differences for each resource in the application in either Compact or Split view modes.| FALSE  |
...
| `restrictedGitSource` | _This Feature Flag is currently in Beta and only enabled for SaaS environments._ We will notify you when you can enable it for on-premises environments. <br>  When enabled, allows you to create a Restricted Git Source in addition to a standard Git Source. <br>See [GitOps: Restricted Git Sources for enhanced application deployment](#gitops-restricted-git-sources-for-enhanced-application-deployment) in this article.| FALSE         |
