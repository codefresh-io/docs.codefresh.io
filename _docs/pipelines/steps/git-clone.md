...
1. Ensure you use the right name for the git integration in your git-clone step.
1. Go to Account Settings (gear icon) -> Pipeline Integrations -> Git.  
   - If you do not have access, please contact one of your team's admins.
1. Ensure that the git Integration allows access to all users.
1. Ensure that the account used for the git integration has the appropriate access.
...
1. Create a new git integration and reference it in the git-clone step.

> **NOTE**  
> When deleting a git integration, you may get an error saying, "The integration cannot be deleted while other objects are using it." When you see this error, it means an existing pipeline/trigger is referencing the integration. Please refer to [How To: Find pipelines using specific Git integration]({{site.baseurl}}/docs/kb/articles/find-pipeline-specific-git-integration) on how to find which pipline is using the git integration.
