...
| `exclude_blob`    | Specifies if to include or exclude blob (Binary Large Object) files from the Git repo being cloned. <br>The options are:<br> `false`: The default, includes the blob files from the Git repo being cloned. <br>`true`: Excludes blob files from the Git repo being cloned. Blob files in the HEAD are always included, while historical versions are not. | Optional      |
