...
| `exclude_blob`    | Specifies if to include or exclude blob (Binary Large Object) objects from the Git repo being cloned. <br>The options are:<br> `false`: The default, includes blob objects from the Git repo being cloned. <br>`true`: Excludes blob objects from the Git repo being cloned. Blob objects in the HEAD are always included, while historical versions are not. | Optional      |
