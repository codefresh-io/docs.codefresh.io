...
| `exclude_blob`    | Specifies if to include or exclude BLOB (Binary Large Object) files from the Git repo being cloned. <br>The options include:<br> `false`: The default value, includes the BLOB files from the Git repo. <br>`true`: Excludes BLOB files from the Git repo. BLOB files in the HEAD are always included, while historical versions are not. | Optional      |
