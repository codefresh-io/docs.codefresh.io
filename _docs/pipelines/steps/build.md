...
## Securing container images by Codefresh pipelines

Singing off on the artifacts crated and distibuted by pipelines and vierifying theiu aitenticity is a key factor in the artifact process.
Sigstore has long been the authorify used 

Private-public key pairs have long been used to both anure and verify the authenicty of container images. Because of the security and maintenance challenges inherent with the key-based method, Sigstore has come up with the concept of keyless signing.
Keyless signing Tis implemented with the OpenID Connect (OIDC) protocol. 

### Securing container images with OIDC and keyless signing
...
| ------------------------------------------                          | ---------------------------------------------- | 
| OIDC provider          | The organization that generates the container images must become an OIDC provider. <br> Codefresh is an official OIDC provider and you can set up integration with Codefresh as an official OIDC provider. See [OIDC in pipelines]({{site.baseurl}}/docs/integrations/oidc-pipelines/).    | Codefresh|
| ID token for provider  | An ID token must be created for the OIDC provider. <br>Codefresh automatically obtains the ID token through a dedicated step that is a part of the build step.  The `obtain-oidc-id-token` step is a dedicated Marketplace step which retrieves an ID token from the Codefresh OIDC provider, enabling authentication and authorization for pipeline actions.No need for any user-action.       | Codefresh|
| ID token verification  | A certificate authrity from sigstore verifies the ID token asignst Codefresh as teh OIDC provuder and issues a short-lived cryptographic certificate based on the ID token.        | Fulcio|
| Signing container image  | The generated container image is signed by an external authority.The `cosign-sign` freestyle step does the following:
...
* Authenticates to the Docker registry where the signature is pushed        | Cosign|
| Authenticity verification  | The certificate (public key) along with signature/digest are packaged and sent to an append-only transparency log. Any external organization can verify the artifact using the information provided in the transparency log.        | Rekor|

#### Adding keyless signing to `build` step
...
Key-based signing is one method of signing artifacts such as container images generated by Codefresh pipelines.

There are two options with key-based signing:
*  Signing without a password
*  Signing with a password/passphrase


Both options use a generated private key and Cosign to sign the container image with the private key, either with password, or requiring a password to first unlock the private key and then sign the container image. 

How does this work in pipelines?
...
cosign: 
  ...
  options: 
    ...
    key-pass: "e2e-test-password"  # only for key-based signing 
```
where:
* `cosign: sign: true` indicates that the Cosign will sign the container image generated by the pipeline.
* `options` can include any of the commands supported by Cosign. 
* `options.key` is path to the private key used to sign the artifact.
* `options.key-pass` is unique to Codefresh and is the password or passphrase to unlock the private key for signing when you require added security with key-based signing.
...
```yaml
  ...
  0_scenario_simple_key_nopass_sign:
      type: "build"
      ...
      cosign:
        sign: true
      ...
      ...
  0_scenario_simple_key_nopass_verify:
      title: Verify key based - empty password
      ...
      commands:
        - cosign verify --key ${{CF_BUILD_ID}}.pub ${{steps.0_scenario_simple_key_nopass_sign.imageId}}
      when:
        steps:
        - name: 0_scenario_simple_key_nopass_sign
          on:
            - success
```
...
```yaml            
  ...
  1_scenario_simple_key_pass_sign:
      type: "build"
      ...
      cosign:
        sign: true
        options:
          key: "${{CF_VOLUME_PATH}}/${{CF_BUILD_ID}}-with-pass.key"
          key-pass: "e2e-test-password"
      ...
  1_scenario_simple_key_pass_verify:
      title: Verify key based - non-empty password
      stage: "simple-with-key-pass"
      image: "bitnami/cosign:2.4.0"
      working_directory: '${{CF_VOLUME_PATH}}'
      commands:
        - cosign verify --key ${{CF_BUILD_ID}}-with-pass.pub ${{steps.1_scenario_simple_key_pass_sign.imageId}}
      when:
        steps:
        - name: 1_scenario_simple_key_pass_sign
          on:
            - success
```
