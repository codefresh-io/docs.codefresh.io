...
| `cache_from`   | The list of cache sources to use as Docker cache when building the image.  If there is more than one cache source, every source in the list is passed as an argument to the `docker_build` command. See [more info](https://docs.docker.com/engine/reference/commandline/buildx_build/#cache-from){:target="\_blank"}.                     | Optional                   |
...
| `retry`   | Define retry behavior for the build step, as described in [Retrying a step]({{site.baseurl}}/docs/codefresh-yaml/what-is-the-codefresh-yaml/#retrying-a-step).  | Optional                  |
