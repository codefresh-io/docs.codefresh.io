...
## Interaction between Argo CD secrets and Git tokens
Codefresh needs access to Git repositories for reading and writing to configuration and resource manifests. This section elaborates on how  Argo CD secrets interact with Git tokens for authentication to Git providers and repositories. 

### Argo CD secrets
Argo CD OSS repository credentials are stored in the `argocd-repo-creds` secret, used exclusively by Argo CD to read files from Git repositories. Argo CD does not support Git commits on behalf of users or individuals.  
...
The Git Runtime token is the personal access token provided during Runtime installation and is automatically converted to a secret. The secret for the Runtime repository is stored in the `runtime-repo-creds-secret` secret, labeled with `argocd.argoproj.io/secret-type: repo-creds`. This label allows Argo CD to use these credentials for all Git read and write operations.  
...
The Git user token, also a personal access token is also converted to a secret. They are stored in the `git-default-<account-id>` secret, encrypted for security. Each user’s token is used for operations initiated by the user via the UI.
