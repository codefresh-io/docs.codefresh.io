...
| `BRANCH`       | The specific Git branch to which to promote changes, retrieved from `spec.source.targetRevision` defined in the application's configuration settings. If `spec.source.targetRevision` is set to `HEAD`, this ris the value that is used for `BRANCH` in both the Pre- and Post-Action Workflows, even if the resolved branch name is `main`, `master`, or any custom name.|✅   | ✅   | 
