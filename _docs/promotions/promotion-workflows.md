...
The table describes the parameters with default values passed in Pre- and Post-Action Workflows. Some parameters are passed to both Promotion Workflows, while some parameters are relevant only to  Post-Action Workflows.
...
|`COMMIT_SHA`| The unique identifier (SHA) of the commit, generated by Git, including the precise set of changes addressed by the commit. Can be used as a trigger condition in Promotion Flows configured for a product. See NIMA: add xref  |Post-Action Workflow |
...
|`COMMIT-MESSAGE` | The context for the changes introduced by the commit. Can be used as a trigger condition in Promotion Flows configured for a product. See NIMA: add-xref |Post-Action Workflow |
...
For Post-Action Workflows, Codefresh passes several parameters generated by the `commit` Promotion Action to the Workflow. The same set of parameters are passed also for pull requests, after the pull request is merged.
...
{: .table .table-bordered .table-hover}
| Parameters                     | Description            | Pre- or Post-Action Workflow | 
| --------------           | --------------           |  
|`APP_NAME`| The specific name of the application, identifying the application the commit and subsequent workflow execution pertain to. For example, `trioapp-staging`.|
|`COMMIT_SHA`| The unique identifier (SHA) of the commit, generated by Git, including the precise set of changes addressed by the commit. Can be used as a trigger condition in Promotion Flows configured for a product. See NIMA: add xref  |
|`COMMIT_AUTHOR`| The name of the user who made the commit. Useful for tracking changes and for notifications.|
|`COMMIT-MESSAGE` | The context for the changes introduced by the commit. Can be used as a trigger condition in Promotion Flows configured for a product. See NIMA: add-xref
