...
description: "Set up Codefresh after installing the on-premises platform"
group: installation
toc: true
---
...
The Codefresh Admin Management panel provides all the options you need for setup. Review the options available in Admin Management, or continue with the steps to complete setting up Codefresh on-premises for your organization:
...
Only those settings required for initial set up are described here. For a description of all Accounts page, see ??
...
1. Continue with [Step 3: Invite a user to a Codefresh account].
...
Once a user is invited, their status remains as Pending until they accept the invitation.

The Users page initially displays the `codefresh-system` user and the `AdminCF` user. 
...
1. To enable username-password login for the user, complete these additional steps:
...
Enable features to open them for Codefresh accounts. You can either enable features for single account, selectively for multiple accounts, or for all accounts. 
...
>**IMPORTANT:**  
Codefresh strongly recommends against enabling the System Features not explicitly mentioned here, unless you are familiar with the implications of the features.  
If you need to do so, we recommend contacting Codefresh support. 
