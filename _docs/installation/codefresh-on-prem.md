...
|`disableWelcomeScreen` | Default: FALSE<br>When enabled, bypasses the Codefresh Welcome screen that requires the user to enter additional infromation on first sign-in. |Required mostly in on-premises environments, especially for LDAP, which has all login info already configured.  |
