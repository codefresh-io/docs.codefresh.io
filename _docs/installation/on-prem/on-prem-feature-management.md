...
|`parallelKubectlOperations` |When enabled, allows running parallel steps including `kubectl`, especially parallel Helm install and `deploy` steps that deploy to multiple clusters with `kubectl`. | FALSE|
