...
where:  
  * `<helm-release-name>` is the name of the Helm release automatically generated by Codefresh, and to be copied from the Codefresh UI.  
  * `<namespace>` is the namespace in which to install the Hybrid GitOps runtime, either `codefresh`, or a custom name you define.
  *  `<codefresh-account-id>` is mandatory only for _tunnel-based Hybrid GitOps Runtimes_ which is the default. It is automatically generated by Codefresh and must be copied from the Codefresh UI.
  * `<codefresh-api-key>` is the generated API key, also to be copied from the Codefresh UI.
  * `<runtime-name>` is the name of the runtime, either `codefresh`, or the custom name you define. 
  * `gitops-runtime` is the chart name defined by Codefresh, also automatically p .
